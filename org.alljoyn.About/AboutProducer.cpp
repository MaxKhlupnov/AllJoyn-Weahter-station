...
std::map<alljoyn_busobject, WeakReference*> AboutProducer::SourceObjects;
std::map<alljoyn_interfacedescription, WeakReference*> AboutProducer::SourceInterfaces;

AboutProducer::AboutProducer(AllJoynBusAttachment^ busAttachment)
    ...
    m_sessionId(0)
{
    ...
    m_signals = ref new AboutSignals();
    m_busAttachmentStateChangedToken.Value = 0;
}

AboutProducer::~AboutProducer()
{
    UnregisterFromBus();
    delete m_weak;
}

void AboutProducer::UnregisterFromBus()
{
...
bool AboutProducer::OnAcceptSessionJoiner(_In_ alljoyn_sessionport sessionPort, _In_ PCSTR joiner, _In_ const alljoyn_sessionopts opts)
{
...
void AboutProducer::OnSessionJoined(_In_ alljoyn_sessionport sessionPort, _In_ alljoyn_sessionid id, _In_ PCSTR joiner)
{
    ...
    {
        AllJoynHelpers::SessionLostHandler<AboutProducer>,
        AllJoynHelpers::SessionMemberAddedHandler<AboutProducer>,
        AllJoynHelpers::SessionMemberRemovedHandler<AboutProducer>
    };
...
void AboutProducer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void AboutProducer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void AboutProducer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void AboutProducer::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {   
        QStatus result = AllJoynHelpers::CreateProducerSession<AboutProducer>(m_busAttachment, m_weak);
        if (ER_OK != result)
        {
            StopInternal(result);
            return;
...
void AboutProducer::CallGetAboutDataHandler(_Inout_ alljoyn_busobject busObject, _In_ alljoyn_message message)
{
    ...
    AboutProducer^ producer = source->second->Resolve<AboutProducer>();
    if (producer->Service != nullptr)
    {
        ...
        AboutGetAboutDataResult^ result = create_task(producer->Service->GetAboutDataAsync(callInfo, inputArg0)).get();
        create_task([](){}).then([=]
        {
            int32 status;
            ...
            status = TypeConversionHelpers::SetAllJoynMessageArg(alljoyn_msgarg_array_element(outputs, 0), "a{sv}", result->AboutData);
            if (AllJoynStatus::Ok != status)
            {
                alljoyn_busobject_methodreply_status(busObject, message, static_cast<QStatus>(status));
...
void AboutProducer::CallGetObjectDescriptionHandler(_Inout_ alljoyn_busobject busObject, _In_ alljoyn_message message)
{
    auto source = SourceObjects.find(busObject);
    if (source == SourceObjects.end())
    {
    ...
    if (producer->Service != nullptr)
    {
        ...
        AboutGetObjectDescriptionResult^ result = create_task(producer->Service->GetObjectDescriptionAsync(callInfo)).get();
        create_task([](){}).then([=]
        {
            int32 status;
            ...
            status = TypeConversionHelpers::SetAllJoynMessageArg(alljoyn_msgarg_array_element(outputs, 0), "a(sas)", result->ObjectDescription);
            if (AllJoynStatus::Ok != status)
            {
                alljoyn_busobject_methodreply_status(busObject, message, static_cast<QStatus>(status));
                alljoyn_msgarg_destroy(outputs);
...
void AboutProducer::CallAnnounceSignalHandler(_In_ const alljoyn_interfacedescription_member* member, _In_ alljoyn_message message)
{
    ...
    auto producer = source->second->Resolve<AboutProducer>();
    if (producer->Signals != nullptr)
    {
        ...
        auto eventArgs = ref new AboutAnnounceReceivedEventArgs();
        ...
        uint16 argument1;
        (void)TypeConversionHelpers::GetAllJoynMessageArg(alljoyn_message_getarg(message, 1), "q", &argument1);
        eventArgs->Port = argument1;
        Windows::Foundation::Collections::IVector<AboutObjectDescriptionItem^>^ argument2;
        (void)TypeConversionHelpers::GetAllJoynMessageArg(alljoyn_message_getarg(message, 2), "a(sas)", &argument2);
        eventArgs->ObjectDescription = argument2;
        Windows::Foundation::Collections::IMap<Platform::String^,Platform::Object^>^ argument3;
        (void)TypeConversionHelpers::GetAllJoynMessageArg(alljoyn_message_getarg(message, 3), "a{sv}", &argument3);
        eventArgs->MetaData = argument3;

        producer->Signals->CallAnnounceReceived(producer->Signals, eventArgs);
    }
}

QStatus AboutProducer::AddMethodHandler(_In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_methodhandler_ptr handler)
{
...
QStatus AboutProducer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
    alljoyn_interfacedescription_member member;
    if (!alljoyn_interfacedescription_getmember(interfaceDescription, methodName, &member))
    {
...
QStatus AboutProducer::OnPropertyGet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _Inout_ alljoyn_msgarg value)
{
...
QStatus AboutProducer::OnPropertySet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _In_ alljoyn_msgarg value)
{
    UNREFERENCED_PARAMETER(interfaceName);
...
void AboutProducer::Start()
{
    ...
    QStatus result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_AboutIntrospectionXml);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    result = AllJoynHelpers::CreateBusObject<AboutProducer>(m_weak);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    alljoyn_interfacedescription interfaceDescription = alljoyn_busattachment_getinterface(AllJoynHelpers::GetInternalBusAttachment(m_busAttachment), "org.alljoyn.About");
    ...
    result = AddMethodHandler(
        ...
        "GetAboutData", 
        [](alljoyn_busobject busObject, const alljoyn_interfacedescription_member* member, alljoyn_message message) { UNREFERENCED_PARAMETER(member); CallGetAboutDataHandler(busObject, message); });
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    result = AddMethodHandler(
        ...
        [](alljoyn_busobject busObject, const alljoyn_interfacedescription_member* member, alljoyn_message message) { UNREFERENCED_PARAMETER(member); CallGetObjectDescriptionHandler(busObject, message); });
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    result = AddSignalHandler(
        ...
        interfaceDescription,
        "Announce",
        [](const alljoyn_interfacedescription_member* member, PCSTR srcPath, alljoyn_message message) { UNREFERENCED_PARAMETER(srcPath); CallAnnounceSignalHandler(member, message); });
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^,AllJoynBusAttachmentStateChangedEventArgs^>(this, &AboutProducer::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void AboutProducer::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void AboutProducer::StopInternal(int32 status)
{
    UnregisterFromBus();
    Stopped(this, ref new AllJoynProducerStoppedEventArgs(status));
}

int32 AboutProducer::RemoveMemberFromSession(_In_ String^ uniqueName)
{
...
PCSTR org::alljoyn::About::c_AboutIntrospectionXml = "<interface name=\"org.alljoyn.About\">"
