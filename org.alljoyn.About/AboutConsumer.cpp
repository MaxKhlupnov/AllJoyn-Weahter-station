...
std::map<alljoyn_interfacedescription, WeakReference*> AboutConsumer::SourceInterfaces;

AboutConsumer::AboutConsumer(AllJoynBusAttachment^ busAttachment)
...
void AboutConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void AboutConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void AboutConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus AboutConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<AboutJoinSessionResult^>^ AboutConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ AboutWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> AboutJoinSessionResult^
    {
        auto result = ref new AboutJoinSessionResult();
        result->Consumer = ref new AboutConsumer(watcher->BusAttachment);
        result->Status = result->Consumer->JoinSession(serviceInfo);
        return result;
    });
}

IAsyncOperation<AboutGetAboutDataResult^>^ AboutConsumer::GetAboutDataAsync(_In_ Platform::String^ interfaceMemberLanguageTag)
{
    return create_async([this, interfaceMemberLanguageTag]() -> AboutGetAboutDataResult^
    {
...
IAsyncOperation<AboutGetObjectDescriptionResult^>^ AboutConsumer::GetObjectDescriptionAsync()
{
    return create_async([this]() -> AboutGetObjectDescriptionResult^
    {
        ...
        {
            result->Status = AllJoynStatus::Ok;
            Windows::Foundation::Collections::IVector<AboutObjectDescriptionItem^>^ argument0;
            status = static_cast<QStatus>(TypeConversionHelpers::GetAllJoynMessageArg(alljoyn_message_getarg(message, 0), "a(sas)", &argument0));
            result->ObjectDescription = argument0;
...
IAsyncOperation<AboutGetVersionResult^>^ AboutConsumer::GetVersionAsync()
{
    return create_async([this]() -> AboutGetVersionResult^
    {
...
void AboutConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
...
void AboutConsumer::CallAnnounceSignalHandler(_In_ const alljoyn_interfacedescription_member* member, _In_ alljoyn_message message)
{
    ...
    auto consumer = source->second->Resolve<AboutConsumer>();
    if (consumer->Signals != nullptr)
    {
        ...
        consumer->Signals->CallAnnounceReceived(consumer->Signals, eventArgs);
    }
}

int32 AboutConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<AboutConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<AboutConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<AboutConsumer>
    };
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "org.alljoyn.About");
