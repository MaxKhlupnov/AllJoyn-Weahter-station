...
AboutWatcher::AboutWatcher(AllJoynBusAttachment^ busAttachment) :
...
AboutWatcher::~AboutWatcher()
{
    UnregisterFromBus();
}

void AboutWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.alljoyn.About" };
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.alljoyn.About"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.alljoyn.About"),
            port);
        Added(this, args);
    }
...
void AboutWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<AboutWatcher>
        };
...
void AboutWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_AboutIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &AboutWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void AboutWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void AboutWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
