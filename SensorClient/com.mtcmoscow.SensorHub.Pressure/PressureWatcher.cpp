...
PressureWatcher::PressureWatcher(AllJoynBusAttachment^ busAttachment) :
...
PressureWatcher::~PressureWatcher()
{
    UnregisterFromBus();
}

void PressureWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.mtcmoscow.SensorHub.Pressure" };
...
void PressureWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.mtcmoscow.SensorHub.Pressure"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.mtcmoscow.SensorHub.Pressure"),
            port);
        Added(this, args);
    }
...
void PressureWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<PressureWatcher>
        };
...
void PressureWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_PressureIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &PressureWatcher::BusAttachmentStateChanged);
...
void PressureWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
