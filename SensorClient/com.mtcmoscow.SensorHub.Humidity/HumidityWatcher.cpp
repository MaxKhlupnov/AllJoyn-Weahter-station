...
HumidityWatcher::HumidityWatcher(AllJoynBusAttachment^ busAttachment) :
...
HumidityWatcher::~HumidityWatcher()
{
    UnregisterFromBus();
}

void HumidityWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.mtcmoscow.SensorHub.Humidity" };
...
void HumidityWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.mtcmoscow.SensorHub.Humidity"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.mtcmoscow.SensorHub.Humidity"),
            port);
        Added(this, args);
    }
...
void HumidityWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<HumidityWatcher>
        };
...
void HumidityWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_HumidityIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &HumidityWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void HumidityWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void HumidityWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
